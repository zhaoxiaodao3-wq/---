...
import { aggregateStats, DIM_OPTIONS, DIM_DAY } from '../../utils/stats';
...
export default function Statistics() {
  const [dim, setDim] = useState(DIM_DAY);
  ...
  const { summary, rows } = useMemo(() => {
    ...
    let payables = getPayables();

    if (dateRange && dateRange.length === 2) {
      const [s, e] = dateRange;
      orders = orders.filter((o) => o.date >= s && o.date <= e);
    }
    ...
    const result = aggregateStats(orders, expenses, payables, dim);
    return {
